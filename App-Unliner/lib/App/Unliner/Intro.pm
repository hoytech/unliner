...
If a pipeline begins with a cat of no arguments, that cat is removed and no cat process is created.

If a pipeline begins with a cat of exactly one argument, then that file is opened and dup2()ed to STDIN.

=item * Trailing cats

If a pipeline ends with a trailing cat, this cat is removed unless STDIN is a terminal. Trailing cats are useful to prevent a program from doing special terminal formatting things like adding ANSI colours.

=item * Internal cats

All internal cats with no arguments are removed. Such cats aren't as silly as they sound. Sometimes a pipeline component will have a leading or trailing cat for some reason. This optimisation will stop any unnecessary cat processes from being created.

=back

Because of the above optimisations, the following program won't start a single cat process:

    def main {
      cat file.txt | cat | cat | cat | wc -l | cat | cat
    }
